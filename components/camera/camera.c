...
esp_err_t camera_init(const camera_config_t* config)
{
	...
	// wip
	s_state->sensor.set_contrast(&s_state->sensor, 1);
...
void camera_print_fb(int color)
{
	/* Number of pixels to skip in order to fit into terminal screen.
	...
	/*for (int ih = 0; ih < s_state->height; ih += pixels_to_skip * 2) {
		for (int iw = 0; iw < s_state->width; iw += pixels_to_skip) {
			printf("index: %d", color + f * iw  + (f * ih * s_state->width));
		}
	}*/

	for (int ih = 0; ih < s_state->height; ih += pixels_to_skip*3) {
		for (int iw = 0; iw < s_state->width; iw += pixels_to_skip*4) {
			if (color == 4)
			{
				px = (s_state->fb[iw + (ih * s_state->width)]);
			} else {
				px = (s_state->fb[color + f * iw + (f * ih * s_state->width)]);
			}

			if (px < 10) printf(" %d ", px);
			else if (px < 100) printf("%d ", px);
			else printf("%d", px);
...
int8_t** img2mtx(int color)
{
	// allocate 2D array memory
	int8_t** blue;
	blue = malloc(s_state->height * sizeof(int8_t*));
	if (blue == NULL)
		return;

	for (int i = 0; i < s_state->height; i++) {
		blue[i] = malloc(s_state->width * sizeof(int8_t ));
		if (blue == NULL)
			return;
	}
	...
	printf("alloc 1: %d \n", s_state->height * sizeof(int8_t*));
	printf("alloc 2: %d \n", s_state->width * sizeof(int8_t ));

	int f = 3;
	int8_t px;

	for (int ih = 0; ih < s_state->height; ih += pixels_to_skip) {
		for (int iw = 0; iw < s_state->width; iw += pixels_to_skip) {
			if (color == 4)
			{
				px = (s_state->fb[iw + (ih * s_state->width)]);
	...
	/*for (int ih = 0; ih < s_state->height; ih += pixels_to_skip * 3) {
		for (int iw = 0; iw < s_state->width; iw += pixels_to_skip * 4) {*/
		/*if (blue[ih][iw] < 10) printf(" %d ", blue[ih][iw]);
				else if (blue[ih][iw] < 100) printf("%d ", blue[ih][iw]);
				else printf("%d", blue[ih][iw]);*/
	/*for (int ih = 0; ih < s_state->height; ih += pixels_to_skip * 2) {
		for (int iw = 0; iw < s_state->width; iw += pixels_to_skip) {		

			if (blue[ih][iw] < 26) printf(" ");
			else if (blue[ih][iw] < 51) printf(".");
			else if (blue[ih][iw] < 77) printf(":");
			else if (blue[ih][iw] < 102) printf("-");
			else if (blue[ih][iw] < 128) printf("=");
			else if (blue[ih][iw] < 154) printf("+");
			else if (blue[ih][iw] < 179) printf("*");
			else if (blue[ih][iw] < 205) printf("#");
			else if (blue[ih][iw] < 230) printf("%%");
			else               printf("@");
		}
		printf("\n");
	}*/
...
int16_t** allocmem()
{
	...
	m = malloc(s_state->height * sizeof(int16_t*));
	if (m == NULL)
		return;

	for (int i = 0; i < s_state->height; i++) {
		m[i] = malloc(s_state->width * sizeof(int16_t));
		if (m == NULL)
			return NULL;
	}

	ESP_LOGI(TAG,"allocmem: %d", sizeof(m));
...
int8_t** allocmem8()
{
	...
	m = malloc(s_state->height * sizeof(int8_t*));
	if (m == NULL)
		return;

	for (int i = 0; i < s_state->height; i++) {
		m[i] = malloc(s_state->width * sizeof(int8_t));
		if (m == NULL)
			return NULL;
	}
...
int8_t** derivativeX(int8_t** m) 
{

	int8_t** hx;
	hx = malloc(s_state->height * sizeof(int8_t*));
	if (hx == NULL)
		return;

	for (int i = 0; i < s_state->height; i++) {
		hx[i] = malloc(s_state->width * sizeof(int8_t));
		if (hx == NULL)
			return;
	}

	for (int i = 0; i < s_state->height; i++) {
		for (int j = 0; j < s_state->width - 1; j++) {
			hx[i][j] = 1 * m[i][j + 1] - 1 * m[i][j];
		}			
	}

	ESP_LOGI(TAG, "DERIVATIVE X");
	return hx;
}

int8_t** derivativeY(int8_t** m) 
{

	int8_t** hy;
	hy = malloc(s_state->height * sizeof(int8_t*));
	if (hy == NULL)
		return;

	for (int i = 0; i < s_state->height; i++) {
		hy[i] = malloc(s_state->width * sizeof(int8_t));
		if (hy == NULL)
			return;
	}

	for (int i = 0; i < s_state->height - 1; i++) {
		for (int j = 0; j < s_state->width; j++) {
			hy[i][j] = 1 * m[i + 1][j] - 1 * m[i][j];
		}
	}

	ESP_LOGI(TAG, "DERIVATIVE Y");

	return hy;
}

int8_t** derivativeT(int8_t** current_frame, int8_t** next_frame)
{
	int8_t** ht;
	ht = malloc(s_state->height * sizeof(int8_t*));
	if (ht == NULL)
		return;

	for (int i = 0; i < s_state->height; i++) {
		ht[i] = malloc(s_state->width * sizeof(int8_t));
		if (ht == NULL)
			return;
	}
	...
	for (int i = 0; i < s_state->height - 1; i++) {
		for (int j = 0; j < s_state->width; j++) {
			ht[i][j] = 1 * current_frame[i][j] - 1 * next_frame[i][j];
		}
	}

	ESP_LOGI(TAG, "DERIVATIVE T");

	return ht;
}

int8_t** saveF(int color)
{
	int8_t** frame;

	frame = malloc(s_state->height * sizeof(int8_t*));
	if (frame == NULL)
		return;

	for (int i = 0; i < s_state->height; i++) {
		frame[i] = malloc(s_state->width * sizeof(int8_t));
		if (frame == NULL)
			return;
	}	
	...
	for (int ih = 0; ih < s_state->height; ih ++) {
		for (int iw = 0; iw < s_state->width; iw ++) {
			if (color == 4)
			{
				px = (s_state->fb[iw + (ih * s_state->width)]);
			}
	...
	//ESP_LOGI(TAG, "SAVE FRAME");

	return frame;
}

int8_t** copy2d(int8_t** m)
{
	int8_t** newm;

	newm = malloc(s_state->height * sizeof(int8_t*));
	if (newm == NULL)
		return;

	for (int i = 0; i < s_state->height; i++) {
		newm[i] = malloc(s_state->width * sizeof(int8_t));
		if (newm == NULL)
			return NULL;
	}
...
void get_neighbors(int h, int w, int8_t** m, int8_t neigh[3][3])
{	
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			neigh[i][j] = m[h + i - 1][w + j - 1];
		}
	}
}

int16_t** f2sum(int8_t** m)
{
	int16_t** msum;
	int8_t n[3][3];

	msum = malloc(s_state->height * sizeof(int16_t*));
	if (msum == NULL)
		return;

	for (int i = 0; i < s_state->height; i++) {
		msum[i] = malloc(s_state->width * sizeof(int16_t));
		if (msum == NULL)
			return;
	}

	for (int ih = 1; ih < s_state->height - 2; ih++) {
		for (int iw = 1; iw < s_state->width - 2; iw++) {

			int16_t sum = 0;
			
			get_neighbors(ih, iw, m, n);
...
int16_t** ffsum(int8_t** A, int8_t** B)
{
	int16_t** msum;
	int8_t n[3][3];
	int8_t m[3][3];
	...
	for (int ih = 1; ih < s_state->height - 2; ih++) {
		for (int iw = 1; iw < s_state->width - 2; iw++) {
			...
			get_neighbors(ih, iw, A, n);
			get_neighbors(ih, iw, B, m);
			...
			msum[ih][iw] = (asum - n[1][1]) * (bsum - m[1][1]);
		}
	}

	/*int pixels_to_skip = 1;
	for (int ih = 1 * 3; ih < s_state->height - 2 * 3; ih += pixels_to_skip * 3) {
		for (int iw = 1 * 3; iw < s_state->width - 2 * 4; iw += pixels_to_skip * 4) {
			if (msum[ih][iw] >= 0)
			{
				if (msum[ih][iw] < 10) printf(" %d  ", msum[ih][iw]);
				else if (msum[ih][iw] < 100) printf(" %d ", msum[ih][iw]);
				else printf(" %d", msum[ih][iw]);
			}
			else
			{
				if (msum[ih][iw] > -10) printf("%d  ", msum[ih][iw]);
...
void uvmatrix(int8_t** fx, int8_t** fy, int8_t** ft)
{
	int8_t** u;
	int8_t** v;
	int8_t** norm;
	...
	for (int ih = 1; ih < s_state->height - 2; ih++) {
		for (int iw = 1; iw < s_state->width - 2; iw++) {
			...
			get_neighbors(ih, iw, fx, n);
			get_neighbors(ih, iw, fy, m);
			get_neighbors(ih, iw, ft, r);
			...
			//if (vector == 1) {
			    u[ih][iw] = (-x2sum * xtsum + xysum * ytsum) / (x2sum * y2sum - pow(xysum, 2));
			/*}
			else
			{*/
				v[ih][iw] = (xysum * xtsum - x2sum * ytsum) / (x2sum * y2sum - pow(xysum, 2));
			//}			
...
void printmatrix(int8_t** uv)
{
	...
	for (int ih = 1 * 3; ih < s_state->height - 2 * 3; ih += pixels_to_skip * 3) {
		for (int iw = 1 * 3; iw < s_state->width - 2 * 4; iw += pixels_to_skip * 4) {
			if (uv[ih][iw] >= 0)
			{
				if (uv[ih][iw] < 10) printf(" %d  ", uv[ih][iw]);
				else if (uv[ih][iw] < 100) printf(" %d ", uv[ih][iw]);
				else printf(" %d", uv[ih][iw]);
			}
			else
			{
				if (uv[ih][iw] > -10) printf("%d  ", uv[ih][iw]);
