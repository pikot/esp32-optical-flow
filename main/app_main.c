...
static void handle_grayscale_pgm_stream(http_context_t http_ctx, void* ctx);
...
void app_main()
{
    ...
    if (s_pixel_format == CAMERA_PF_GRAYSCALE) {
		...
		ESP_ERROR_CHECK(http_register_handler(server, "/pgm_stream", HTTP_GET, HTTP_HANDLE_RESPONSE, &handle_grayscale_pgm_stream, NULL));
		ESP_LOGI(TAG, "Open http://" IPSTR "/pgm_stream for multipart/x-mixed-replace stream of pgms", IP2STR(&s_ip_addr));
    }
...
static void handle_rgb_bmp(http_context_t http_ctx, void* ctx)
{
    ...
    if (err != ESP_OK) {
		...
		fy = derivativeY(blue);	
		deallocmem8(blue);
		/*get_neighbors(100, 25, fx, n);
		printf("SUM %d\n", f2sum(n));*/

		ESP_LOGI(TAG, "SIGMAFX");
		sigmafx = f2sum(fx);
		ESP_LOGI(TAG, "SIGMAFY");
		sigmafy = f2sum(fy);

		ESP_LOGI(TAG, "SIGMAFXFY");
		sigmafxfy = ffsum(fx, fy);		
		
		deallocmem(sigmafxfy);	
		deallocmem(sigmafy);
		deallocmem(sigmafx);
		deallocmem8(fx);
		deallocmem8(fy);
...
static void handle_rgb_bmp_stream(http_context_t http_ctx, void* ctx)
{
	...
	int8_t** curframe = NULL;
	int8_t** nextframe = NULL;
	int fcount = 1;
    ...
    while (true) {
        ...
        if (err != ESP_OK) {
			...
			{
				deallocmem8(curframe);
				curframe = copy2d(nextframe);
				deallocmem8(nextframe);
				nextframe = saveF(b);
				ft = derivativeT(curframe, nextframe);
...
static void handle_grayscale_pgm_stream(http_context_t http_ctx, void* ctx)
{
	...
	while (true) {
		...
		err = http_response_begin_multipart(http_ctx, "image/x-portable-graymap",
			camera_get_data_size() + sizeof(*pgm_header_str));
		if (err != ESP_OK) {
			break;
		}
		err = http_response_write(http_ctx, &pgm_header);
		if (err != ESP_OK) {
			break;
		}
		err = write_frame(http_ctx);
